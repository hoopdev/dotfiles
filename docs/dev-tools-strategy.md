...
`dev`（Rust workspace: `pkgs/dev-core` / `pkgs/dev-cli` / `pkgs/dev-tui` /
`pkgs/dev-zellij`）を他の類似ツールと比較し、どこに投資し何をやらないかをまとめた
戦略メモ。使い方は [dev-tools.md](dev-tools.md)、現行リファクタ計画は
...
設計原理: **`dev … --json` を外部契約とし、Rust 内部では `dev-core` の型付き API を使う。**
LLM、TUI、Zellij は同じ概念を扱うが、実行環境に応じて in-process API と JSON subprocess を使い分ける。

- **L0 — `--json` 基盤**: `dev targets`、`ls/info/status/ps/snapshot` に `--json`（既定の人間出力は不変）。外部 automation と Zellij plugin の契約
...
- **可視化**: `dev tui`（Rust ratatui TUI, `pkgs/dev-tui`, `dev-core` を主データ経路にし、端末操作だけ `dev` に shell out）+ `dev board`（Zellij plugin, `dev snapshot --json`）+ `dev dash`（fzf 簡易版）
