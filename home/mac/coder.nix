...
let
  ...
  devTui = pkgs.rustPlatform.buildRustPackage {
    ...
    cargoBuildFlags = [ "--package" "dev-tui" ];
    cargoTestFlags = [ "--package" "dev-tui" ];
    # notify (used for file watching) links against kqueue on macOS — no extra inputs needed
  };

  devCli = pkgs.rustPlatform.buildRustPackage {
    pname = "dev-cli";
    version = "0.1.0";
    src = pkgs.lib.cleanSource ../../pkgs;
    cargoLock.lockFile = ../../pkgs/Cargo.lock;
    cargoBuildFlags = [ "--package" "dev-cli" ];
    cargoTestFlags = [ "--package" "dev-cli" ];
    # dev-core has git2 with vendored-libgit2 — needs cmake to build libgit2 from source
  ...
  devCmd = pkgs.writeShellScriptBin "dev" ''
    ...
    case "$group" in
      ...
      # Task orchestration — delegated to Rust binary
      task)
        exec ${devCli}/bin/dev task "$@"
        ;;
